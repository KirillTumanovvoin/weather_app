...
def get_clothing_recommendation(
    ...
    if temp < -30:
        clothing_list = [  # ← БЫЛО items, СТАЛО clothing_list
    ...
    # ── Сильный мороз (−30 … −10 °C) ──────────────────────
        ...
        if is_snow:
            clothing_list.append("Непромокаемые бахилы или гамаши")
        tip = (
            "Одевайтесь слоями: термобельё → утеплитель → "
    ...
    # ── Холодно (−10 … +5 °C) ──────────────────────────────
        ...
        if is_rain or is_snow:
            clothing_list.insert(2, "Водонепроницаемая верхняя одежда")
            clothing_list.append("Зонт или капюшон")
        tip = (
    ...
    # ── Прохладно (+5 … +15 °C) ────────────────────────────
        ...
        if is_rain:
            clothing_list.insert(2, "Водонепроницаемый плащ или куртка")
            clothing_list.append("Зонт")
        if time_of_day in ("вечер", "ночь"):
            clothing_list.insert(0, "Лёгкий тёплый свитер (вечером холоднее)")
        tip = (
            "Возьмите куртку с собой — "
    ...
    # ── Тепло (+15 … +25 °C) ───────────────────────────────
        ...
        if is_rain:
            clothing_list.append("Лёгкий дождевик или зонт")
        if season == "весна" or time_of_day in ("вечер", "ночь"):
            clothing_list.insert(2, "Лёгкая кофта или кардиган (на вечер)")
        tip = (
    ...
    # ── Жарко (+25 °C и выше) ──────────────────────────────
        ...
        if is_rain:
            clothing_list.append("Компактный зонт")
        tip = (
            "Не забывайте наносить солнцезащитный крем (SPF 30+) "
    ...
    return {
        "list": clothing_list,  # ← ИЗМЕНИЛИ: items → list
        "tip": tip,
        "emoji": emoji,
        "level": level,
    }
